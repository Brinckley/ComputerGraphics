...
public abstract class CGLabDemoOGL : OGLApplicationTemplate<CGLabDemoOGL>
{
    ...
    [DisplayNumericProperty(32, 1, "Specular Power", 1, 100)]
    public abstract int SpecularPower { get; set; }

    [DisplayCheckerProperty(false, "Grid use")]
    public virtual bool Grid { get; set; }

    [DisplayCheckerProperty(true, "Shader use")] 
    ...
    [DisplayCheckerProperty(false, "Vertex buffer use")]
    ...
    protected override void OnMainWindowLoad(object sender, EventArgs args)
    {
        ...
        RenderDevice.AddScheduleTask((gl, s) =>  // all render parameters 
        {
        ...
        // on project closing buffer delete
        ...
        // updating matrix on start and resize
    ...
    private void UpdateModelViewMatrix()
    {
        RenderDevice.AddScheduleTask((gl, s) =>
        {
            ...
            var deg2rad = Math.PI / 180; // camera rotation around the object
    ...
    public void Count_Vertex() // creating array + calculating amount of vertices 
    {
        SIZE = 0;
    ...
    public void Create_Vertex()
    {
        ...
        // bottom side
        double h2 = Heights[1];
        double r2 = Math.Sqrt(ABC[2] * ABC[2] - h2 * h2) / ABC[2];
        for (int j = 1; j < Parameter_Height; j += numbers[Aprox + 3], BottomCircles++)
        {
            CntVertInBottomCircles = 0;
            for (int phi = 0; phi <= 360; phi += numbers[Aprox + 3], CntVertInBottomCircles++)
        ...
        // other sides
        for (int j = 0; j <= cnt_step; j++, Circles++)// Counting all the circles
        {
            CntVertInCircle = 0;
    ...
    public int Polygons = 0; // Polygon number
    public void Count_Polygons()
    {
        Polygons = 0;

        // all upper + bottom polygons 
    ...
    public void Create_Polygons()
    {
        ...
        // dividing upper sides in polygons
        ...
        // dividing bottom sides in polygons
        ...
        // splitting sides into triangles
        for (int j = 1; j < Circles; j++)
        {
    ...
    public void Count_Normal(int a, int b, int c)
    {
        ...
        normal = normal / Math.Sqrt(normal.X * normal.X + normal.Y * normal.Y + normal.Z * normal.Z); // normalization 
